...
use crate::{
    ...
    repositories::audit_logs::{
        org_specificlog_asc, org_specificlog_desc, specific_logs_asc, specific_logs_desc,
    },
...
pub async fn org_logs_service(
    ...
    let data = match order {
        "asc" => org_specificlog_asc(&pool, org_id, decode_cursor, limit).await?,
        _ => org_specificlog_desc(&pool, org_id, decode_cursor, limit).await?,
    };
